...
| Item | Default path |
| --- | --- |
| Application snapshot | `${XDG_DATA_HOME:-$HOME/.local/share}/byok-bridge` |
| User data | `$HOME/.byok-bridge` |
| Command shim | `$HOME/.local/bin/byok` |
| Sourceable Bash library | `${XDG_DATA_HOME:-$HOME/.local/share}/byok-bridge/shell/bash/byok-bridge.bash` |
| Installer-internal profile manager | `${XDG_DATA_HOME:-$HOME/.local/share}/byok-bridge/libexec/linux/bash-profile-manager.mjs` |
| Managed Bash startup block | `$HOME/.bashrc` |
| Optional extension | `${COPILOT_HOME:-$HOME/.copilot}/extensions/byok-bridge-copilot` |

Custom paths must be absolute, safe, and non-overlapping:

```bash
bash bin/linux/install.sh \
  --install-dir /opt/byok-bridge \
  --data-dir "$HOME/.config/byok-bridge" \
  --bin-dir "$HOME/.local/bin"
```

The resolved data directory is recorded in the managed install manifest and exported by the generated shim as `BYOK_BRIDGE_DATA_DIR`, so the Manager and extension read the same state/cache. The sourceable Bash library is part of the application snapshot; the external bin directory contains only the executable `byok` shim. Application, shim, managed `.bashrc` block, extension, and installer-created data changes are rolled back together if a later validation or switch step fails.
...
source shell/bash/byok-bridge.bash
byok
```
...
A managed update must keep the data, shim, and extension paths recorded by the existing manifest. Windows rejects a different `BYOK_BRIDGE_DATA_DIR` or `COPILOT_HOME`; Linux rejects different `--data-dir` or `--bin-dir` values. To relocate these paths without leaving orphaned managed files, uninstall first and then reinstall with the new locations.

Upgrading a managed `0.0.3` installation automatically migrates the application, data directory, command shim, Bash startup block, optional extension, and Windows user `PATH` to the new names. The old tree is retained until the new snapshot passes its checks; a failed migration restores the prior tree byte-for-byte. An already-existing new target or any unowned legacy path is rejected rather than merged or overwritten.
